...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Derivatives simulation (for feature set completeness)
    # In a real system, these would be joined from a separate F&O table.
    # We simulate them as rolling proxies to allow the model to learn their impact.
    out["pcr_ratio"] = out["volatility_5d"] * 10 + 0.8  # Proxy for sentiment-driven hedging
    out["iv_rank_level"] = out["volatility_20d"].rolling(60).rank(pct=True) * 100
