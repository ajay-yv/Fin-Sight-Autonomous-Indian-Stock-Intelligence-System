...
export default function IntelligenceFeed({ runId }: { runId: string }) {
      ...
      try {
        ...
        for (const [key, agent] of Object.entries(agents)) {
          ...
          if (prev?.status !== agent.status && agent.status) {
            ...
            // Allow Macro signals to be seen in the Signals feed
            const verdict = dataVerdict || agent.signal || undefined;
  ...
  const filteredEvents = events.filter((e) => {
    ...
    if (filter === "SIGNALS") {
      // Show actionable signals (BUY/SELL/INSUFFICIENT) OR the final synthesis verdict
      const isActionable = e.verdict && ["BUY", "SELL", "INSUFFICIENT_DATA", "SUPPRESSED"].includes(e.verdict);
      const isSynthesis = e.agent === "SYNTHESIS";
      return isActionable || isSynthesis;
    }
    if (filter === "DATA") return e.status === "started" || e.agent === "DATA_INGESTION";
    return true;
  });
