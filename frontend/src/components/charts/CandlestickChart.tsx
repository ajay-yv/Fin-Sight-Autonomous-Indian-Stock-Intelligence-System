...
export default function CandlestickChart({
  ...
  const fetchChartData = useCallback(async (targetSymbol: string, allowRetry = true, rethrowOnError = false) => {
    ...
    // Safeguard: If we are in a /run route but runId is missing, wait.
    if (!runId && window.location.pathname.includes("/run/")) {
      return;
    }

    // If we have non-empty data passed via props, use it and stop here.
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`request_failed_${res.status} at ${endpoint}`);
      }
      ...
      let finalMessage = "Could not load price data.";
      if (errorName === "AbortError" || errorMessage.includes("timeout")) {
        finalMessage = "Data fetch timed out. Retrying...";
      } else if (errorMessage.includes("request_failed_404") || errorMessage.includes("empty")) {
        finalMessage = "Symbol not found or no historical data available.";
      } else if (errorMessage.includes("request_failed_")) {
        finalMessage = `Server error (${errorMessage.split('_').pop()}). Retrying...`;
      }

      showChartError(finalMessage);
  ...
  }, [data, renderChart, clearRetryTimeout, showChartError, runId, normalizeFetchedData, retryAfter]);
