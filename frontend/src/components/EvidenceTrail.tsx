                ...
                <div className="evidence-triggers triggers flex-1 overflow-visible p-3 font-serif text-sm text-black leading-snug">
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-dashed border-[var(--color-war-border)]">
                      <p className="font-mono text-[9px] uppercase tracking-widest text-[var(--color-war-text)] mb-2">RAW AGENT EXPORT</p>
                      {lm.data ? (
                        <div className="bg-white p-2 border border-[var(--color-war-border)] max-h-[150px] overflow-y-auto">
                           <pre className="text-[10px] font-mono text-[#444] leading-tight">
                             {JSON.stringify(lm.data, null, 2)}
                           </pre>
                        </div>
                      ) : (
                        <p className="font-mono text-[9px] uppercase tracking-widest text-[var(--color-war-muted)]">
                           [DATA UNAVAILABLE FOR {lm.agent}]
                        </p>
                      )}
                    </div>
                  )}
