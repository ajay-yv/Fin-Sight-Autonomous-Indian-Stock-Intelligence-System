...
import SSAPIntelligence from "@/components/SSAPIntelligence";
import RetailDarkPool from "@/components/RetailDarkPool";
...
export default function WarRoomPage() {
  ...
  const [activeTab, setActiveTab] = useState<"intelligence" | "execution" | "ssap" | "darkpool" | "gmss" | "biofeedback">("intelligence");
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-48px)]">
      ...
      <div className="flex bg-[var(--color-war-border)] h-8 border-b border-[var(--color-war-border)] items-center justify-between pr-px overflow-hidden">
        <div className="flex h-full gap-px">
          {runStatus && runStatus.symbols.map((sym) => (
            <button
              key={sym}
              onClick={() => setActiveSymbol(sym)}
              className={`px-4 h-full font-mono text-[10px] uppercase tracking-widest transition-colors ${
                activeSymbol === sym && activeTab === "intelligence"
        ...
        <div className="flex h-full gap-px">
          <button
            ...
            className={`px-6 h-full font-mono text-[10px] uppercase font-bold tracking-widest transition-colors ${
              activeTab === "intelligence"
                ? "bg-[var(--color-war-text)] text-white"
                : "bg-white text-[var(--color-war-text)] hover:bg-[#eee]"
            }`}
          >
            INTELLIGENCE
          </button>
          <button
            ...
            className={`px-6 h-full font-mono text-[10px] uppercase font-bold tracking-widest transition-colors ${
              activeTab === "execution"
                ? "bg-[var(--color-war-buy)] text-white"
                : "bg-white text-[var(--color-war-text)] hover:bg-[#eee]"
            }`}
          >
            AMAPR EXECUTION
